...
Public Class SERVICIOS
    ...
    Public Function VERIFICARULTIMOUSUARIOCONPERMISOSDEDERESET(OBJETO As BE.Usuario) As Boolean
        Try
            Dim DTUSUARIOPATENTE As DataTable = DAL.Conexion.GetInstance.leer("select US.IDUSUARIO from usupat AS UP JOIN USUARIO AS US ON UP.IDUSUARIO = US.IDUSUARIO where UP.IDPATENTE= 1 AND UP.NEGADO = 0 AND US.CANT_INT < 3 AND US.IDUSUARIO <> " & OBJETO.IdUsuario)
            If DTUSUARIOPATENTE.Rows.Count > 0 Then
                Return True
            End If

            Dim DTUSUARIOFAMILIAPATENTE As DataTable = DAL.Conexion.GetInstance.leer("Select US.IDUSUARIO from FAMPAT AS FP JOIN USUFAM AS UF ON FP.IDFAMILIA = UF.IDFAMILIA JOIN USUARIO AS US ON UF.IDUSUARIO=US.IDUSUARIO WHERE FP.IDPATENTE = 1 And US.CANT_INT < 3 AND US.IDUSUARIO <> " & OBJETO.IdUsuario)
